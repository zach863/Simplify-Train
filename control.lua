...
local function safe_get_network(stop, wire_id)
  local ok, network = pcall(stop.get_circuit_network, stop, wire_id)
  if ok then
    return network
  end
...
local function get_stop_network(stop)
  if not (defines and defines.wire_connector_id) then
    return nil
  end

  return safe_get_network(stop, defines.wire_connector_id.circuit_red)
    or safe_get_network(stop, defines.wire_connector_id.circuit_green)
end
